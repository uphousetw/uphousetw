...
const DATA_FILE = path.join(process.cwd(), 'lib', 'data', 'about-data.json');
...
async function loadAbout() {
  try {
    ...
    console.log('📝 File not found, returning default data (Vercel read-only environment)');
    // On Vercel, we can't write files, so just return default data
    const newAboutData = createDefaultAbout();

    // Only try to save if not in Vercel environment
    if (process.env.VERCEL !== '1') {
      try {
        await saveAbout(newAboutData);
        console.log('✅ Successfully created new about-data.json with complete data structure');
      } catch (saveError) {
        console.log('⚠️  Could not save file, returning default data:', saveError.message);
      }
    }
...
export async function updateAboutData(updates) {
  ...
  // On Vercel without MongoDB, we can't persist changes
  if (process.env.VERCEL === '1') {
    throw new Error('File system updates are not supported on Vercel. Please configure MongoDB.');
  }
